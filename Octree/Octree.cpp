...
void Octree::set(int L, double x, double y, double z, double X, double Y, double Z, int* PrimList, int PrimListSize, int* VertList, int VertListSize)
{
	Level = L;
...
void Octree::start(int L, double x, double y, double z, double X, double Y, double Z, ThreeDModel* obj)
{
	Level = L;
...
bool Octree::collide(float x, float y, float z)
{
	// finds octree child at max depth
	bool childrenCollide = false;
	if (Level >= MAX_DEPTH)
	{
		// if child has part of the terain within it 
		if (VertexListSize > 0)
		{
			//return if the sub collide with this
			return x >= minX && x <= maxX && y >= minY && y <= maxY && z >= minZ && z <= maxZ;
		}
	}
	else
	{
		//else finds it there are any child and does the same test 
		for (int i = 0; i < 8; i++)
		{
			if (children[i] != NULL)
			{
				childrenCollide = children[i]->collide(x, y, z);
				// if one of these does collide return true
				if (childrenCollide)
				{
					return true;
				}
